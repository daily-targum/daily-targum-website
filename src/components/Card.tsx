...
function Clickable({
  ...
  return href ? (
    ...
    <button
      ...
      className={cn(className, styles.clickableButton)}
      onClick={onClick}
      role="article"
    >
      {children}
    ...
    >
      ...
      <div className={styles.compactCardBody}>
        ...
        <Text className={styles.byline}>
          {date}
          {date && author ? " - " : null}
          {author ? <Text className={styles.author}>{author}</Text> : null}
        </Text>
      </div>
    </Clickable>
  );
...
function CardCompactResponsiveRelated({
  ...
  return (
    ...
    >
      {/* Desktop */}
      <Grid.Display xs={false} md={true} className={styles.compactCardImageV2}>
        <AspectRatioImage
      ...
      {/* Mobile */}
      <Grid.Display xs={true} md={false} className={styles.compactCardImageV2}>
        <AspectRatioImage
      ...
      <div className={styles.compactCardBodyRelated}>
        {tag ? <Text className={styles.tag}>{tag}</Text> : null}
        {title ? (
          <Text.Truncate
            variant="h4"
...
function CardCompactResponsive({
  ...
  return (
    ...
    >
      {/* Desktop */}
      <Grid.Display xs={false} md={true} className={styles.compactCardImage}>
        <AspectRatioImage
          aspectRatio={aspectRatioDesktop ?? 1}
          data={imageData}
          style={{ minHeight: "100%" }}
      ...
      {/* Mobile */}
      <Grid.Display xs={true} md={false} className={styles.compactCardImage}>
        <AspectRatioImage
          aspectRatio={aspectRatioMobile ?? 1}
          data={imageData}
          style={{ minHeight: "100%" }}
...
export function CardStackedResponsive({
  ...
  return (
    ...
    >
      <>
        {/* Desktop */}
        <Grid.Display xs={false} md={true} style={{ flex: 1 }}>
          <AspectRatioImage
  ...
  return (
    ...
    >
      ...
      <div className={styles.imageCardTitleWrap}>
        ...
        {title ? (
          <Text.Truncate variant="h3" htmlTag="h2" numberOfLines={2}>
            {title}
          </Text.Truncate>
        ) : null}
        <Text className={styles.byline}>
          ...
          {date && author ? " - " : null}
          {author ? <Text className={styles.author}>By {author}</Text> : null}
        </Text>
      </div>
    </Clickable>
  );
...
function CardImageResponsive({
  ...
  return (
    <>
      {/* Desktop */}
      <Grid.Display xs={false} md={true} style={{ height: "100%" }}>
        <CardImage
          {...rest}
