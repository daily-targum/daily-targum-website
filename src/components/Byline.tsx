...
import { formatDate, formatDateAbbreviated } from "../utils";
  ...
  return (
    <>
      <div className={classNames.row}>
        {compact ? null : (
          <aside className={classNames.aside} aria-hidden={true}>
            {authors.map((author) => (
              <Link
                key={author.id}
        ...
        <div className={classNames.column}>
            ...
            {authorsExceptLast.map((author, i) => (
              <React.Fragment key={author.id}>
                <Link
                  ...
                  rel="author"
                >
                  <Text className={classNames.author}>
                    {author.displayName}
                  </Text>
                </Link>
                {i < authorsExceptLast.length - 1 ? (
                  <Text className={classNames.breakSpaces}>, </Text>
                ) : null}
              </React.Fragment>
            ))}
          ...
          <Text className={classNames.date} htmlTag="time" noPadding>
            {compact
              ? formatDateAbbreviated(publishDate)
  ...
  return (
    <>
      <div className={classNames.bylineCompact}>
        <Text htmlTag="time" className={classNames.bylineCompactDate}>
          {formatDateAbbreviated(publishDate)}
        </Text>
